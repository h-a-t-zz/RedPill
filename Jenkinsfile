...
stage 'Building env'
  ...
  def app_param = "--label traefik.backend='app-${state}' --label traefik.port='80' --label traefik.protocol='http' --label traefik.weight='10' --label traefik.frontend.rule='Host:${state}.www.h5f.eu' --label traefik.frontend.passHostHeader='true' --label traefik.priority='10' -e BUILD_STAGE=${state}"
...
stage 'Test with OWASP ZapProxy'
  ...
  stage 'QA'
  input "Is https://${state}.www.h5f.eu going according to plan?"
  ...
  // Defines bb param
  state = "bb"
  app_param = "--label traefik.backend='app-${state}' --label traefik.port='80' --label traefik.protocol='http' --label traefik.weight='10' --label traefik.frontend.rule='Host:${state}.www.h5f.eu' --label traefik.frontend.passHostHeader='true' --label traefik.priority='10' -e BUILD_STAGE=${state}"
